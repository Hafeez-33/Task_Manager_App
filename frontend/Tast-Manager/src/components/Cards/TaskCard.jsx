  ...
  return (
    <div className="bg-white rounded-xl py-4 shadow-md shadow-gray-100 border border-gray-200/50 cursor-pointer" onClick={onClick}>
      <div className="flex items-center gap-3 px-4">
        ...
        <div
          className={`text-[11px] font-medium ${getPriorityTagColor()} px-4 py-0.5 ro`}
        >
          {priority} Priority
        </div>
      </div>
      ...
      >
        ...
        <p className="text-[13px] text-gray-600/80 font-medium mt-2 mb-2 leading-[18px]">
          Task Done:{""}
      ...
      <div className="px-4">
        <div className="flex items-center justify-between my-1">
          <div>
            ...
            <p className="text-[13px] font-medium text-gray-800">{moment(createdAt).format("Do MMM YYYY")}</p>
          </div>

          <div>
            <label className="text-xs text-gray-500">Due Date</label>
            <p className="text-[13px] font-medium text-gray-800">{moment(dueDate).format("Do MMM YYYY")}</p>
          </div>
        </div>
        <div className="flex items-center justify-between mt-3 mb-2">
          ...
          {attachmentCount > 0 && (
            <div className="flex items-center gap-2 bg-blue-50 px-2.5 py-1.5 rounded-lg ">
              <LuPaperclip className="text-xs text-gray-900" />
              {""}
