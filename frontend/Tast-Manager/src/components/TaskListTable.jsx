...
const TaskListTable = ({ tableData }) => {
  const gerStatusBadgeColor = (status) => {
    switch (status) {
      case "Completed":
  ...
  return (
    <div className="overflow-x-auto p-1 rounded-lg mt-3">
      <table className="w-full">
        ...
        <tbody key={Array.isArray(tableData) ? tableData.length :0}>
          {(Array.isArray(tableData) ? tableData:[]).map((task) => (
            <tr key={task._id} className="border-t border-gray-200">
              ...
              <td className="py-4 px-4">
                <span
                  className={`px-2 py-1 text-xs rounded inline-block ${gerStatusBadgeColor(task.status)}`}
                >
                  {task.status}
                </span>
              </td>
